...
class Document:
	...
	def CreateDocumentRels(self):
		...
		for part_name in self.GetParts().keys():
	...
	def Save(self):
		...
		try:
			os.makedirs(self.GetRutaPlantilla())
		except:
			pass

		try:
			zout = ZipFile(self.GetRutaPlantilla() + self.GetPlantilla(), 'w')
		except IOError as e:
		...
		for part in self.parts.items():
			...
			try:
				...
				os.makedirs(self.GetRutaPlantilla() + '/'.join(part_split[:-1]))
			except Exception:
				pass

			if hasattr(part, 'GetImages'):
				for img in part.GetImages():
					zout.writestr('word/' + img[0], img[1])

			with open(self.GetRutaPlantilla() + part.get_name(), 'w') as file_part:
				file_part.write(part.get_xml())
		zout.writestr(self.GetContentTypes().get_name(), self.GetContentTypes().get_xml())

		with open(self.GetRutaPlantilla() + self.GetContentTypes().get_name(), 'w') as file_content:
			file_content.write(self.GetContentTypes().get_xml())
		zout.close()

	# noinspection PyBroadException
	def Load(self):
		path = self.GetRutaPlantilla() + '/' + self.GetPlantilla()
		...
		for item in zin.infolist():
			...
			file_ = path.pop(-1)
			path = self.GetRutaPlantilla() + 'temp/' + '/'.join(path) + '/'
			try:
				os.makedirs(path)
			except Exception:
				pass
	...
	def get_xml(self):
		...
		# value.append('<w:%s %s>' % (self.GetTag(), ' '.join(self.attributes)))
